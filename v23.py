...
def train_fn(train_loader, model, criterion, optimizer, epoch, scheduler, scaler, device):
...
def valid_fn(valid_loader, model, criterion, scaler, device):
	#batch_time = AverageMeter()
	#data_time = AverageMeter()
	losses = AverageMeter()
	scores = AverageMeter()
...
def train_loop(folds, fold):
	...
	train_processed_path = "../data/train_images_raw_ROI_"+str(CFG.resize_to[1])
	valid_processed_path = "../data/train_images_raw_ROI_"+str(CFG.resize_to[1])
	...
	for epoch in range(CFG.epochs):
		...
		avg_loss = train_fn(train_loader, model, criterion, optimizer, epoch, scheduler, scaler, device)
		avg_val_loss, preds = valid_fn(valid_loader, model, criterion, scaler, device)
